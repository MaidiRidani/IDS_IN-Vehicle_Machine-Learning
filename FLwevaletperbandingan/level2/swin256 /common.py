...
def save_best_f1_model(model, best_weights):
    ...
    for (k, old_tensor), w in zip(current_state.items(), best_weights):

        new_state_dict[k] = torch.tensor(
            w,
    ...
    save_model(
        model,
        f"saved_models/model_best_f1_{mode_name}.pt"
    )
...
def save_best_fnr_model(model, best_weights_fnr):
    ...
    for (k, old_tensor), w in zip(current_state.items(), best_weights_fnr):
    ...
    save_model(
        model,
        f"saved_models/model_best_fnr_{mode_name}.pt"
    )
